...
import immich_ml.sessions.qnn as qnn_session
...
class InferenceModel(ABC):
    ...
    def _make_session(self, model_path: Path) -> ModelSession:
        ...
        match model_path.suffix:
            ...
            case ".onnx":
                # Optional: route specific models to the Qualcomm NPU via the
                # qnn-dsp-daemon (active only when IMMICH_ML_QNN_URL is set and
                # the model is covered by a pre-built QNN context binary).
                key = qnn_session.model_key_for(self.model_type, self.model_task, self.model_name)
                session = qnn_session.QnnSession(model_path, key) if key else OrtSession(model_path)
            case ".rknn":
