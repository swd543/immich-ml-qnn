...
Usage:
  tools/stage_image_assets.sh --sdk <QAIRT-2.37.1.250807-root> \
    ...
    --scrfd <scrfd_6490_v2.bin> [--runtime-root <board-runtime-root>]
...
Stages the exact Docker build inputs:
  ...
  daemon/models/{clipr37_6490.bin,arcface37v6_6490.bin,scrfd_6490_v2.bin}
...
sdk= clip= arcface= scrfd= runtime_root=
while (($#)); do
  case "$1" in
    --sdk) sdk=${2:?}; shift 2 ;;
    --clip) clip=${2:?}; shift 2 ;;
    --arcface) arcface=${2:?}; shift 2 ;;
    --scrfd) scrfd=${2:?}; shift 2 ;;
...
[[ -n $sdk && -n $clip && -n $arcface && -n $scrfd ]] || { usage >&2; exit 2; }
...
[[ -f $scrfd ]] || { echo "missing SCRFD context: $scrfd" >&2; exit 1; }
...
install -m 0644 "$scrfd" daemon/models/scrfd_6490_v2.bin
